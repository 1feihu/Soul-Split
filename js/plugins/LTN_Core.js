 ...
 * @author LTN Games
 * @version 1.2.0 - Slippery Snake
 */
// =============================================================================
// LTN_Core.js
// =============================================================================
/*:
 @plugindesc v 1.2.0 LTN Core Utility Plugin. Place above all LTN plugins.
 ...
 @param Fit Browser Window
 @parent Resolution Options
 @type boolean
 @desc Will adjust the game resolution to fit the web browser.
 @default false

 @param Disable Resolution
 @parent Resolution Options
 @type boolean
 @desc Disables the Core from resizing and changing resolution.
 @default false
 ...
 @param Print Deprecated
 @parent Debug Options
 @type boolean
 @desc Prints deprecated method warnings for methods which are now deprecated
 @default false
...
(function ($) {
   ...
   *
   ------------------------------------------------------------------------ */
  LTN.Core.Parameters = PluginManager.parameters('LTN_Core');
  LTN.Core.Param = LTN.Core.Param || {};

  var Parameters = LTN.Core.Parameters;
  var Param = LTN.Core.Param;

  /* Resolution Settings */
  Param.screenWidth = Number(Parameters['Screen Width'] || 816);
  Param.screenHeight = Number(Parameters['Screen Height'] || 624);
  Param.fitToBrowser = Boolean(Parameters['Fit Browser Window'] === 'true' || false);
  Param.disableResolution = Boolean(Parameters['Disable Resolution'] === 'true' || false);
  ...
  Param.renderChoice = String(Parameters['Force Renderer']);
  ...
  Param.printDeprecated = Boolean(Parameters['Print Deprecated'] === 'true' || false);
  Param.skipTitle = Boolean(Parameters['Skip Title'] === 'true' || false);
  Param.fpsStart = Boolean(Parameters['Show FPS'] === 'true' || false);

  /* Screenshot Settings */
  Param.screenshotKey = Number(Parameters['Screenshot Key'] || 44);
  Param.screenshotPath = String(Parameters['Screenshot Path']);
  Param.screenshotQuality = Number(Parameters['Screenshot Quality'] / 100 || 0.7);

  /* Bugfix Setting */
  Param.vehicleBGMFix = Boolean(Parameters['Vehicle BGM Fix'] === 'true' || false);
  ...
  var Utilities = function Utilities() {
    return {
      ...
      */
       ...
       */
      hexToRgb(hex) {
        var interger = parseInt(hex, 16);
        var r = interger >> 16 & 255;
        var g = interger >> 8 & 255;
        var b = interger & 255;
      ...
      /**
       * Parses an object with JSON.parse and stringify essentialy shallow cloning an object
       *
       ...
       * @param {Function} action - The callback function to evaulate
       */
       ...
       *
       * @param {*} text - The text to evaulate
      ...
      /**
       * Scan all events on map and extract thier comments
       *
      ...
      /**
       * Takes a screenshot by getting the canvas/renderer's data.
       *
       * @param {any} quality
       * @returns
       *
  ...
  var FileIO = function FileIO() {
    ...
    return {
       ...
       * @param {any} destination - The destination to save the downlaoded file.
...
;(function (JsonEx) {
  ...
  JsonEx.structParse = function (text) {
    ...
    for (var key in obj) {
      ...
      } else if (LTN.Utilities.isBool(obj[key])) {
        obj[key] = LTN.Utilities.toBool(obj[key]);
      }
    }
    return obj;
  };
...
(function (DM, Alias) {
  ...
  /**
   * Assign save data to original data refrence in plugin object.
   *
  ...
  var assignSaveData = function assignSaveData(contents) {
    ...
    /* we only need to save data that exists in 'contents' */
 ...
 * PluginManager >>
 *
 * Retireve plugin params by description tag <Tag>
 ...
 *
 ------------------------------------------------------------------------ */
(function (SM) {
  var rendererChoice = LTN.Utilities.toObj(LTN.Core.Param.renderChoice);
  ...
  SM.run = function (sceneClass) {
    ...
    SM.openDebugTools();
    if (LTN.Core.Param.startFullScreen) {
  ...
  SM.resizeToResolution = function () {
    if (!LTN.Core.Param.disableResolution) {
    ...
    if (LTN.Core.Param.fitToBrowser && !Utils.isNwjs()) {
...
(function ($, Param) {
  ...
  $._onKeyUp = function (event) {
    ...
    if (event.keyCode === LTN.Core.Param.screenshotKey && Utils.isNwjs()) {
      var date = new Date();
      var image = LTN.Utilities.takeScreenshot(Param.screenshotType, Param.screenshotQuality);
      var name = `${date.getMonth()}_${date.getDate()}_${date.getFullYear()}`;
      LTN.FileIO.saveImage(image, Param.screenshotPath, `Screenshot_${name}`);
    }
  };
})(Input, LTN.Core.Param)
 ...
 *
 ------------------------------------------------------------------------ */
(function ($) {
  var scaleBacks = LTN.Utilities.toObj(LTN.Core.Param.scaleBacks);
...
Game_Map.prototype.tileWidth = function () {
  return LTN.Core.Param.tileSize;
};
...
/** -----------------------------------------------------------------------
 * Scene_Map  >>
 * Add CoreSignal for when the game map is loded. Signals, events created and map ready
 *
 ------------------------------------------------------------------------ */
 ...
 * Scene_Boot  >>
 *
 * options of skipping the title on bootup when playtesting a project.
 *
 *------------------------------------------------------------------------ */
...
LTN.PluginRegistrar.registerPlugin('Core', '1.2.0', 'LTN Games');