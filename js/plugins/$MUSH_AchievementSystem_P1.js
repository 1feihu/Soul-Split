...
/*:
 * 
 * @plugindesc [v.1.03] Plugin runs the MushroomCake28 Achievement System
 * @author MushroomCake28
...
Imported.mushFeatures['AchievementSystem_P1'] = 1.03;

var $mushFeatures = $mushFeatures || { 'imported': {}, 'params': {} };
$mushFeatures.imported['AchievementSystem_P1'] = 1.03;
...
MUSH_Achievements.prototype.createSceneDetails = function() {
	...
	this._snDetails = {};
		...
		//   - One line Max.
		//-----------------------------------------------
		this._snDetails.sceneDescription = "Press OK to toggle the scene view!";
...
MUSH_Achievements.prototype.createAchievementsList = function() {
	...
	//-------------------------------------------------------------------------------
		...
		["Test2",					0, "Package1_1", "Test2 Switch 31",							 "-1/0/0",	"0/31/0", false, false, false] ,
...
Scene_Map.prototype.update = function() {
	...
	if (this._wPopPeriod > 0) {
		if (Graphics.frameCount % this._wPopPeriod == 0) {
			this.verifyAchievementCompletion();
		}
	}
};
...
Window_MenuCommand.prototype.addAchievementCommand = function(dtAch) {
	if (this.needsCommand(dtAch.sceneTitle)) {
        var enabled = true;
        this.addCommand(dtAch.sceneTitle, 'mushAchievements', enabled);
